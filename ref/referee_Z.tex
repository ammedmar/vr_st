...
\begin{document}
	...
	\begin{enumerate}
		...
		\ling{Results on spheres hold with $\Z$ (e.g. Section 4.3.1).
		...
		\anibal{I am ok with that. Please proceed.}
		...
		To establish the stronger bound $\beta_i^j \le \delta_{j-1}$, we would need to prove a new base case showing that the $n_1$-th homological radius of $\bS^{n_2}_G$ is bounded above by the filling radius of $\bS^{n_{1}}_G$, which do not see how to prove.

		\ling{I do not immediately see if this new base holds, and I prefer to not think too hard about it. How should we address this comment better?}

		\anibal{I added a sentence at the end. It looks OK to me as is.}
		...
		\ar We added the following explanation to the text:

		\smallskip

		\begin{quote}
			We note that the left-most vertical map is an isomorphism.
			Since no cells are introduced in dimensions $\le n_{j-1}$
			when passing from $\bS^{n_{j-1}}$ to $\bS^{n_j}$,
			the same holds for the quotient spaces.
			Therefore the induced map on homology is an isomorphism
			in degrees $< n_{j-1}$ \ling{how to prove the isomorphism in degree $n_{j-1}$ in the general case?}.
		\end{quote}

		\anibal{I am thinking that maybe this lemma should be restricted to the real and complex equatorial systems with their \(\cyc_2\) and \(\cyc_q\) actions.
