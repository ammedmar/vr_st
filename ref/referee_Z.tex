...
\begin{document}
	...
	\begin{enumerate}
		...
		\ar We thank the reviewer for pointing out that the map $\mathrm{VR}(X)/G \to \mathrm{VR}(X/G)$ is always well defined, whereas defining its inverse requires the strong $r$-diameter condition. We have revised the formula on page 20 accordingly:
		\begin{quote}
			\begin{align*}
				h_r \colon \VR_r(\cX)_G &\to  \VR_r(\cX_G) \\
				\textstyle \big[\sum\lambda_i x_i\big] & \mapsto \textstyle\sum\lambda_i [x_i].
			\end{align*}
			If the action is proper and by isometries, as well as satisfies the \textit{strong \(r\)-diameter} condition, a concept we review below, then for any \(s \leq r\), the map $h_s$ is an isomorphism of simplicial complexes.
		\end{quote}
