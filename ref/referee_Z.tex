...
\begin{document}
	...
	\begin{enumerate}
		...
		\begin{quote}
			...
			For orientable manifolds (e.g., spheres and lens spaces), the choice of coefficients does not affect the filling radius, as noted in \cite{gromov1983filling}.
			We adopt these conventions throughout and omit \(\field\) from the notation when clear from the context.
		\end{quote}
		...
		\begin{quote}
			Although different choices of \(\field\) does not affect the filling radius, it may affect the \(m\)-homological radius. 
			However, for the results presented in this work, the choice of field (specifically \(\Ftwo\) for non-orientable manifolds and any field for orientable manifolds) does not affect the statements, so we omit it from the notation when clear from context.
		\end{quote}

		\smallskip

		In \S4.7, we added a reminder that the ground field considered here is arbitrary:
		
		\smallskip

		\begin{quote}
			Here we work with any field as the ground field.
		\end{quote}
		...
		In this case, the isomophism is a well-known fact.
		...
		\ar The original proof separated the odd and even cases by assuming integer coefficients.
		We observed that Lemma 4.5.3 does not depend on homology and thus is independent of the choice of coefficients, and Lemma 4.5.4 holds as long as coefficients are chosen consistently across the spaces.
		Consequently, we modified the proof in \S4.7 to work with general coefficients, treating the odd and even cases uniformly and providing a more concise argument.
