...
% \textcolor{red}{By \cite{adamaszek2020homotopy}, the Vietoris--Rips filtration of a wedge sum is naturally homotopy equivalent to the wedge sum of the corresponding filtrations, and by \cite{lim2024vietoris}, an analogous result holds for products. 
% Leveraging these results, we study persistent cohomology operations under sums and products.
% }
...
The \defn{gluing metric} on the wedge sum of pointed metric spaces ${(\cX_i, x_i)}_{i = 0}^k$ is defined as follows \cite{burago2001course}: For $x \in \cX_i$ and $x' \in \cX_j$ their distance is
\[
d(x, x') =
\begin{cases}
