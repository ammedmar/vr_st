...
For the round Lens space $\rL^n_p$ of radius \(\pi\), Desideratum (3) is achieved by selecting $\theta$ as an appropriate Steenrod reduced power (see \cref{sss:cohomology_lens}).
Desideratum (1) remains unproven, but for Desideratum (2) it suffices to check that
\[
\fillrad(\rL^n_p) < \tfrac{\zeta_{2n+1}}{4} + \tfrac{\zeta_{m}}{2} \text{ and is non-decreasing as a function of \(n\)}.\anibal{What is \(m\)? This is not a math statement. The \(n\) was fixed, but now is a parameter.}
\]
...
In this subsection we provide even sharper bounds for the corresponding interleaving distances.
Explicitly, if \(\bS_{\rp^n} = \bS^1 \vee \dots \vee \bS^n\) as before, we have the following.
...
As \(n\) grows, \(\tfrac{\pi - \zeta_n}{2}\) increases and approaches \(\tfrac{\pi}{4}\).\anibal{It seems that the inequality became sharper in the wrong direction tho?}