...
\medskip We verify that the round real projective spaces of radius \(\pi\) satisfy these Desiderata.
...
\medskip For fixed $n \in \N$ and the round Lens space $\rL^n_p$ of radius \(\pi\), Desideratum~(3) is achieved by selecting $\theta$ as an appropriate Steenrod reduced power (see \cref{sss:cohomology_lens}).
...
%
%\begin{proof}
%	Since $-\frac{1}{n+1} < 0$ we have $\zeta_n = \arccos\!\left(-\frac{1}{n+1}\right) > \arccos(0) = \frac{\pi}{2}$.
%	Also $\zeta_1 = \arccos\!\left(-\frac{1}{2}\right) = \frac{2\pi}{3}$.
%
%	(1) The first inequality follows from $\zeta_m - \zeta_n \le \zeta_1 - \zeta_n < \tfrac{2\pi}{3} - \tfrac{\pi}{2} = \tfrac{\pi}{6} < \tfrac{\pi - \zeta_n}{2}$.
