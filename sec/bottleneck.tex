...
For any fixed coefficient field $\field$ (omitted from the notations below),
\begin{enumerate}
    \item $\crit(\cM) \geq \tfrac{\zeta_n}{2} $;
    \item $\firstdeath{m}{\cM} < \tfrac{\zeta_n}{4}+\tfrac{\zeta_m}{2}$ for any $m \in \N$.
    \anibal{Do you need all $m$ or just the one related to the non-zero cohomology operation? \ling{I think we need all $m$, because our point is there is one cohomology operation that works better than all persistent homology. If we do not do so, it could be that at some $m$ persistent homology still does a good or even better job.} }
    \item There exists a linear cohomology operation $\theta$ such that $\img_\theta(\cM)$ is non-zero. 
\end{enumerate}

As shown in ... real projective spaces satisfy these.
For Lens spaces ...
% This follows from the facts that (1) $\crit(\rp^n) = \tfrac{\pi}{3}$, (2) $\firstdeath{m}{\rp^n} = \tfrac{\pi}{3}$ for $m \leq n$ and $0$ otherwise, and (3) $\rp^n$ admits non-trivial Steenrod square operations. 
% Facts (1) and (2) were established in \cref{s:first_critical_value_rpn}. 

\ling{I think the right place to write this is in the proof of position 4.6 (2). I have improved that part. Please take a look.}\anibal{OK}

% More precisely, $\VS \defeq \VS^{u_1, \dots, u_n}$ with $u_i = \dim \opH_i(\cM)$ for each $i = 1,\dots, n$.

% We establish the following result to show that the interleaving distance between persistent image $\theta$-modules can provide a better (lower-bound) approximation of the Gromov--Hausdorff distance between $\cM$ and $\VS$ than the interleaving distance between the persistent homology.

\subsubsection{}

Let $\VS$ be the wedge sum of unit spheres with the same reduced homology groups as $\cM$, which we assume satisfies the desiderata.\anibal{What is the radius? This should depend on $\cM$, no? \ling{We requested $\cM$ to have diameter $\pi$, so the radius is one here.}}
\anibal{What do you think about the notation $\bS_\cM$ \ling{Looks good to me. We will need both $\bS_\cM$ and $\bS_{\rp^n}$. If you also think they both look good, I can make the changes.}}
...
\begin{equation}\label{eq:db_theta_lower_bound}
    ...
    %\db\big(\thetabarc{\VS}, \thetabarc{\cM}\big)
	%= \min_Q \cost(Q)
...
\medskip\theorem\anibal{Is this corollary? \ling{I do not think `corollary' is suitable because it is not the consequence of the general comparasion. It proves something stronger in the case of $\rp^n$. It can be a proposition.}}
