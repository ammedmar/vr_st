...
\begin{enumerate}
    ...
    \item $\firstdeath{m}{\cM} < \tfrac{\zeta_n}{4}+\tfrac{\zeta_m}{2}$ for any $m \in N$.\anibal{is this the right scale? I would move the 2 to the RHS.\ling{fixed.}}
    \item There exists a linear cohomology operation $\theta$ such that $\img_\theta(\cM)$. \lz{This sentence is incomplete.}
\end{enumerate}

\anibal{Ling: please explain how the lemma in the previous section can be used to get (2) from increasing fill rad. and reference the subsection on Lens spaces for (3). Also, one needs to fix a ring for this discussion. Here also one needs to foreshadow the computations in the next section where this desiderata is verified for real projective spaces.}
...
\anibal{Ling, this still speaks of barcodes. ALso, it should be $\cM$ not $\cM$. The irst is a manifold, the second a (metric) space. \ling{fixed.}}
