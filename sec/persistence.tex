...
Changing the target category $\cC$ \ling{add the word `functorially'} does not increase the interleaving distance.
...
\ling{I changed the bottleneck distance definition to be free from intervals and barcodes.}
...
\subsection{Vietoris--Rips complexes and stability}
...
\ling{Tranditionally, $\VR(\cX)$ is called `Vietoris-Rips filtration'. Then, use the term `Vietoris-Rips complex at a scale $r$' to refer to $\VR_r(\cX)$}
