...
Two $\R$-spaces $X$ and $Y$ are \defn{weakly equivalent}, denoted by $X \simeq Y$, if there exists an $\R$-space $W$ and morphisms $f \colon W \to X$ and $g \colon W \to Y$ that are weak equivalences.\footnote{
...
In this paper, we will not distinguish a simplicial complex from its geometric realization.

The Vietoris--Rips thickening, often denoted as $\VR_r^m(X)$, is introduced in \cite{adamaszek2018metric} as a metric space analogue of $\VR_r(X)$. 
For a metric space $X$ and $r>0$, the \defn{Vietoris-Rips thickening} is 
\label{para:VR thickening}
\[
    \VR_r^m(X) \defeq \{\sum_{i=1}^k\lambda_i\delta_{x_i}\mid k\geq 2, \lambda_i\geq 0, \sum \lambda_i = 1, \diam(\{x_1,\dots, x_k\})<r\}, 
\]
where $\delta_{x}$ is the Dirac probability measure with mass one at $x$ and $\VR_t^m(X)$ is equipped with the $1$-Wasserstein metric. 
There is a set bijection from (the geometric realization of) $\VR_r(X)$ to $\VR_r^m(X)$, given by $\sum_{i=1}^k\lambda_i x_i\mapsto \sum_{i=1}^k\lambda_i\delta_{x_i}.$ 
We will refer to a point $\sum_{i=1}^k\lambda_i\delta_{x_i}$ in the Vietoris--Rips thickening simply as $\sum_{i=1}^k\lambda_i x_i$, when the meaning of the notation is clear from the context. 

It is shown in \cite[Theorem 1]{gillespie2024vietoris} that the natural bijection from $\VR_r(X)$ to $\VR_r^m(X)$ is a weak homotopy equivalence. 
...
%\anibal{Check references please, since now we a citation to the published version.}
...
In a similar way, one can define the filling radius over $\k$, denoted \(\fillrad[\kk]{\cM}\).

\medskip\lemma(\cite[Prop.9.28]{lim2020vietoris}). If $\cM$ be a closed connected $n$-dimensional Riemannian manifold, then \(\Hbarc{n}{\cM}\) contains \((0,2\fillradnofield{\cM})\) for arbitrary field if $M$ is orientable, and for mode $2$ coefficients if $M$ is nonorientable.
Moreover, this is the unique interval in \(\Hbarc{n}{\cM}\) starting at $0$.

\anibal{Is this the right citation? I am surprised to see $\Ftwo$.}