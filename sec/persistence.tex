...
Two $\R$-spaces $X$ and $Y$ are \defn{weakly equivalent}, denoted by $X \simeq Y$, if there exists an $\R$-space $W$ and weak equivalences $X \leftarrow W \rightarrow Y$.\footnote{
...
Then, for any $k \in \N,$
\[
\di(\rH_k(\VR(\cX)), \rH_k(\VR(\cY))) \leq
\dhi(\VR(\cX), \VR(\cY)) \leq
2 \cdot \dgh(\cX, \cY).
\]

\begin{proof}
	These inequalities follow from Theorem.~1.9 and Theorem.~1.10 in \cite{blumberg2023interleaving}.\anibal{I do not see the compactness assumption here}
\end{proof}
