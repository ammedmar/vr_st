...
\subsubsection{}
Throughout, we think of the set of real numbers $\R$ as a category via its poset structure.

For any category $\cC$, the category $\cC^\R$ of \defn{$\R$-diagrams} in $\cC$ is that of functors from $\R$ to $\cC$ together with and natural transformations between them.
...
For any $\R$-diagram $F \colon \R \to \cC$, we write $F^\delta$ for the $\R$-diagram
\[
...
\ling{Should we cite this paper: \url{https://link.springer.com/article/10.1007/s10208-014-9229-5}?}
\ling{It is the first paper defining interleaving for general category and proving the lemma below (see Theorem on page 1508).}
...
Two $\R$-spaces $X$ and $Y$ are \defn{weakly equivalent}, denoted by $X \simeq Y$, if there exists an $\R$-space $W$ and morphisms $f \colon W \to X$ and $g \colon W \to Y$ that are weak equivalences.
...
\subsubsection{}
\theorem(\cite[Thm.~5.14]{chazal2016stability}) The interleaving distance of $q$-tame persistence modules agrees with the bottleneck distance of its barcodes.
...
\subsubsection{}
For any $k \geq 0$, applying the degree $k$ homology functor (with coefficients in $\kk$) to an $\R$-space produces the persistence module $\rH_k(X; \kk)$.
%, where the morphisms are those induced by $\set{X_{r,s}}_{r \leq s}$.
...
For an $n$-dimensional Riemannian manifold $\cM$, its \defn{filling radius} is defined as the infimal $\epsilon > 0$ such that the fundamental class in $\opH_n(M;\mathbb{Z})$ is mapped to zero under the induced map of the inclusion $\cM \hookrightarrow U_\epsilon(\cM)$ \cite[page 108]{gromov2007metric}.
...
\proposition If $\cM$ be a closed connected $n$-dimensional Riemannian manifold, then \(\Hbarc[\kk]{n}{\cM}\) contains \((0,2\fillrad[\kk]{\cM})\), when $\kk$ is an arbitrary field if $\cM$ is orientable and $\kk=\Ftwo$ if $\cM$ is non-orientable.

\begin{proof}
	This statement is proven as Proposition~9.28 in \cite{lim2020vietoris}.
\end{proof}