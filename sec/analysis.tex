...
\subsection{Barcodes of $\bbS^n$}\label{ss:Sn}
...
\subsubsection{} We study the Steenrod barcodes of $\bbS^n$.
...
\subsubsection{} Using Example \ref{ss:Sn} and Proposition \ref{prop:wedge sum} (\ref{prop:wedge sum pH}), we obtain an estimate of the barcodes of $\VS^n$ defined as $\bbS^1  \vee  \bbS^2 \vee \dots \vee \bbS^n$, illustrated in the top row of Figure \ref{fig:barcodes}.
Explicitly, we have the following:
\begin{itemize}
	\item $\Hbarc{1}{\VS^n}$ contains only one bar $\left(0,\frac{2\pi}{3}\right)$.
...
\begin{figure}
	...
	\caption{\emph{Top row:} barcodes of $\VS^n = \bbS^1 \vee \bbS^2 \vee \dots \vee \bbS^n$. \emph{Bottom row:} barcodes of $\rp^n$.}
	\label{fig:barcodes}
\end{figure}
...
\medskip\theorem
\begin{itemize}
	\item[(1)] $\max_{p\geq 1}\db\left(\Hbarc{p}{\VS^n}, \Hbarc{p}{\rp^n}\right)\leq \frac{\pi-\zeta_n}{2}<\frac{\pi}{4}$.
	\smallskip\item[(2)] $\db\left(\sqbarc{k}{\VS^n}, \sqbarc{k}{\rp^n}\right)\geq \frac{\pi}{3},\forall 1\leq k\leq n-1$.
\end{itemize}
