...
\anibal{The commented out stuff below is not clear to me. A equivariant map inducing a weak equivalence is not necessarily a weak equivalence on orbits. I added the conclusion to the definition of \(\VR\)-compatible action}
% \subsubsection{}\label{subsub:rho}

% \medskip\lemma
% For any $r \in (0, \zeta_n)$, the following composition is a weak homotopy equivalence
% \[
...
A \defn{\(G\)-action} on an equatorial system consists of a \(G\)-action on each sphere commuting that commutes with the isometric embeddings.
...
We say that a \(G\)-action on an equatorial system is \defn{\(\VR\)-compatible} if the \(G\)-action on each sphere is \(\VR\)-compatible and the following diagram commutes for every \(i \in \N\):
\[
\begin{tikzcd}
	\VR_r(\bS^{n_i})_G
	\ar[d]
...
\begin{proof}
	\anibal{Here is where the proof hidden below should go, using the monotonicity of the filling radius as in my email (now written as a lemma early in the paper)}
\end{proof}
