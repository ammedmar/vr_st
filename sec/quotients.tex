...
The case of $\cyc_2$ in the following lemma was established in \cite[Cor.~4.3]{adams2022metric}.
...
% 	Then, for each $i \in \{1, \dots, k\}$, there exists $g_i \in \cyc_q$ such that $d_{\bS}(x_0, g_i x_i) < r$. Let $x_i^* = g_i x_i$. We claim that $d_{\bS}(x_i^*, x_j^*) < r$ for all $1 \leq i, j \leq k$.
...
\begin{proof}
	...
	For any $i \leq j-1$, applying the degree $n_i$ reduced homology functor to diagram \eqref{eq:fundamental_bars_diagram} and using the fact that the $\cyc_2$-action on the system is $\VR$-compatible (cf. \cref{ss:system VR compatible}), we obtain a commutative diagram of vector spaces:
