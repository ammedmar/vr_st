...
%\[
%\beta_{n_j}(\bS^{n_j}_G) \leq 2 \cdot \Rad(\bS^{n_j}_G)
%\]
%where \(\beta_{n_j}(\bS^{n_j})\) is the first critical value of \(\rH_{n_j}(\VR \bS^{n_j}_G)\).

\medskip\lemma If the free \(G\)-action in the above equatorial system is \(\VR\)-compatible and $\Rad(\bS^{n_i}_G)$ is non-decreasing as a function of \(i\) then, for any \(i \leq j\),
\[
\firstdeath{n_i}{\bS^{n_j}_G} \leq \Rad(\bS^{n_j}_G).
\]

\begin{proof}
    To simplify notation, for any $i \leq j$, let
    \[
        \cX_j = \bS^{n_j}_G, \,\delta_j = \Rad(\cX_j) \text{ and }\beta_i^j = \firstdeath{n_i}{\cX_j}.
    \]
