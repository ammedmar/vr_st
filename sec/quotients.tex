...
A \(G\)-action on \(\bS^n\) which is both proper and by isometries is said to be \defn{admissible} if it commutes with \(f_r^n\) and the induced quotient,
\[
\tilde f_r^n \colon \VR_r(\bS^n)_G \to \bS^n_G,
\]
is a weak equivalence for $r < \zeta_n$.\anibal{why was \(r=0\) excluded?}
...
\begin{proof}
	...
	\]
	According to \cite[Proposition 5.3]{adamaszek2018metric}, when $0<r<\zeta_n$, $f \circ g^{-1}$ is a homotopy equivalence, implying that $f = (f \circ g^{-1}) \circ g$ is a weak homotopy equivalence.\anibal{should be \(f_r^n\)}
\end{proof}
...
\anibal{Continue from here with this notation. Are there additional assumptions needed?}
