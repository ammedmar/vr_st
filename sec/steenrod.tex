...
Such space is unique up to homotopy, and is referred to as an \defn{Eilenberg--MacLane space} of type $\pi,n$.
...
The \defn{Hurewicz homomorphism} $h \colon \pi_i(X) \rightarrow \rH_i(X)$ is defined by choosing a generator $u$ of $\rH_i(S^i)$ and sending the homotopy class $[f]$ of a based map $f \colon \bbS^i \to X$ to $f^*(u)$.
...
The relevance of Eilenberg--MacLane space for cohomology is the following bijection
\[
...
A \defn{stable cohomology operation of degree $k$} is a natural family of functions
\[
...
We will denote by $\cO^\st_k(\pi, G)$ the set of stable cohomology operation of degree $k$.
...
Using representability, $\cO^\st_k(\pi, G)$ can be described as the colimit of the groups
\[
...
\[
\cO^\st(\pi) = \bigsqcup_k \cO^\st_k(\pi, \pi),
\]
which defines a (graded) algebra with product given by composition.

When $\pi$ is $\Z/p$ for some prime $p$, the algebra of stable cohomology operations, known as the \defn{mod $p$ Steenrod algebra}, is of central importance in homotopy theory.
In the computational part of this paper, we will focus on the mod 2 case, referring to $\cO^\st(\Z/2)$ simply as the Steenrod algebra.
...
The quotient space under this action, denoted by \(\rp^n\), is the \defn{real projective $n$-space}.
...
The actual Steenrod algebra is generated by the \defn{Steenrod squares} $\set{\Sq^k}_{k \in \N}$, with $\Sq^k$ a degree $k$ stable operation and $\Sq^0 = \id$.
...
%Explicitly,
%\[
%\cO^\st(\Z/2) \cong \frac{(\Z/2)[\Sq^0,\Sq^1,\dots]}{(\Sq^0 = 1, \text{Adem})}.
%\]
...
If $X$ is q-tame, we define its \defn{$\theta$-barcode} as the barcode of $\img(\theta)$.
