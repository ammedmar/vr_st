...
\medskip\proposition \anibal{Please rewrite in terms of \(\crit\). I don't even think we need \(\zeta\). Here is best to express in terms of filling radius}
...
From the previous proposition we know that for \(r \in (0, \zeta_n)\) the spaces \(\VR_r(\bS^n)\) and \(\bS^n\) are homotopy equivalent for any \(n \in \N\).
We now recall from \cite{bibid} a natural map realizing this equivalence.\anibal{reference?}

For \(n \in \N\) and \(r < \pi\), the \defn{\(\VR_r(\bS^n)\)-projection}
\[
f_r^n \colon \VR_r(\bS^n) \to \R^{n+1} \setminus \set{0} \to \bS^n
\]
is the composition of the map sending a formal linear combination $\sum\lambda_i x_i$ to the point \(\sum\lambda_i x_i\) in \(\bbR^{n+1}\) and the map sending a point in \(\R^{n+1} \setminus \set{0}\) to its radial projection.\anibal{I would like a comment about why this is well defined.}
