...
A persistence module \(M\) is termed \defn{ephemeral} if all the maps \(V_{s,t} \colon V_s \to V_t\) are zero for \(s < t\).
The \defn{radical} \(\rad M\) of a persistence module \(M\) is defined as the unique minimal submodule of \(M\) such that the cokernel of the inclusion \(\rad M \hookrightarrow M\) is an ephemeral persistence module.
...
An \defn{$\R$-diagram of spaces} $X$ is a functor from $\R$ to the category $\Top$ of topological spaces with continuous maps.
...
The space $\colim X$ will play a role below.
An important class of $\R$-diagrams is one-parameter filtrations of a space.
