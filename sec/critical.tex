...
By \cite[Thm.~3.5]{hausmann1995vietoris} and \cite[Thm.~4.1]{lim2020vietoris}, we know that there is a \(r(\cM) > 0\) such that for all \(r \in (0,r(\cM))\) the spaces \(\rU_r(\cM)\) and \(\cM\) are homotopy equivalent.
We define the \defn{first critical radius} of \(\cM\), denoted by \(\crit(\cM)\), as the supremum among all such real numbers.

Clearly, if \(\cM\) is connected we have
\[
\crit(\cM) \leq \fillrad(\cM).
\]
...
\subsubsection{}\label{subsub:beta v.s. fillrad}

%Let us assume \(\cM\) connected.
%Since, as stated in \cref{ss:kuratowski_vr}, \(\VR_{2r}(\cM)\) is homotopy equivalent to \(U_r(\cM) \subset \rL^\infty(\cM)\), an upper bound for \(\crit(\cM)\) is \(2\cdot\fillrad(\cM)\).
%
