...
%Clearly, if \(\rH_m(\cM) \neq 0\),
%\[
%\Rad_{htpy}(\cM) \leq 2 \cdot \Rad_m(\cM).
%\]
...
%and observe that
%\[
%\Rad_{htpy}(\cM) \leq 2 \cdot \Rad_\theta(\cM))
%\]
%if \(\img_\theta(\cM) \neq 0\).
...
%Since, as stated in \cref{ss:kuratowski_vr}, \(\VR_{2r}(\cM)\) is homotopy equivalent to \(U_r(\cM) \subset \rL^\infty(\cM)\), an upper bound for \(\Rad_{htpy}(\cM)\) is \(2\cdot\Rad(\cM)\).
%
...
\medskip\lemma
If $\cM$ is a closed connected $n$-dimensional Riemannian manifold, then \(\Hbarc{n}{\cM}\) contains the bar \((0,2{\cM})\) for arbitrary field coefficients if $M$ is orientable, and for mod $2$ coefficients $M$ if not.\anibal{Fix interval please}
