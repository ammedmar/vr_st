...
Any compact metric space $\cX$ can be isometrically embedded into the space $\rL^\infty(\cX)$ of all bounded real-valued functions on $\cX$ via the map $x \in \cX \mapsto d_\cX(x,\cdot)$, the so-called \defn{Kuratowski embedding} which enjoys certain valuable universal properties.
%\footnote{
%This embedding is used because of the following universal property: an arbitrary distance-decreasing map of a subspace \(\cY \subset \cZ\) of a metric space into \(\rL^\infty(\cX)\), extends to a distance-decreasing map \(\cZ \to \rL^\infty(\cX)\).}
...
\medskip We will use this equivalence to bound the Vietoris--Rips barcode of Riemannian (pseudo-)manifolds using their critical radii.
