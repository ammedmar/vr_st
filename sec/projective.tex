...
%We respectively denote by $\crit(\rp^n), \crit_m(\rp^n)$ and $\crit_{\Sq_\ell^k}(\rp^n)$ the first critical values of \(\VR(\rp^n)\), \(\rH_\degp(\VR(\rp^n))\), and \(\img_{\Sq^k_\ell}\).

In \cref{subsub:rpn homotopy type}, \cref{subsub:beta_m_rpn} and \cref{subsub:gamma_rpn}, we will respectively prove that $\crit(\rp^n), \crit_m(\rp^n)$ and $\crit_{\Sq_\ell^k}(\rp^n)$ are all $\frac{2\pi}{3}$, when $1 \leq \degp \leq n$, $0 \leq k \leq \min\{\frac{n-1}{2}, \ell\} \leq n$ and $\binom{\ell}{k}$ is odd.

Applying results in \cref{subsub:barcode_general}, we obtain an estimate of the barcodes of $\rp^n$ and illustrate it in \cref{fig:sq barcodes}, as this will be needed for estimating the bottleneck distance between barcodes.
...
\begin{proof}%[Proof of \cref{subsub:beta_m_rpn}.]
    ...
    On the other hand, $\crit_m(\rp^n)\geq \crit(\rp^n) = \tfrac{2\pi}{3}$.
...
\begin{proof}
	...
	On the other hand, $\gamma_{k+\ell} \geq \crit(\rp^n) = \tfrac{2\pi}{3}$ the first critical value of $\VR(\rp^n)$.
