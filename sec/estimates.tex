...
Its \defn{first critical value} is the supremum among real numbers $r$ such that for any
% $0 \leq s \leq t \leq \r$, $\VR_t(\cX) \simeq \cX$ and $\VR_s(\cX) \hookrightarrow \VR_{t}(\cX)$ is a homotopy equivalence

\anibal{I am changing this def since it should be better express for more general $\R$-spaces}
Similarly, we define the \defn{first critical value of a persistence module $V$} to be some $\beta \geq 0$ such that the structure maps before $\beta$ are isomorphisms and $V_r$ changes isomorphism type at $r = \beta$.
\anibal{I replaced $\beta$ by $\beta$ since $m$ is playing no role in this definition.}

%\l{spaces are $\cX$. parameters are $r$, instead of $t$. $\degp$ change to $m$}
\medskip\example
...
For any $1 \leq \ell, \degp \leq n$, let $\theta \in \cO(\ell, \degp; \kk)$. 
