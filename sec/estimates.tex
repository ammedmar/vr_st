...
\begin{figure}
	...
	\caption{\emph{Top row:} usual barcodes of $\rp^n$. \emph{Bottom row:} Steenrod barcodes of $\rp^n$. Each figure applies to all positive degrees. \ling{update the degrees}}
...
The \defn{first critical value of $\VR(X)$} is some $\alpha\geq 0$ such that 
\begin{itemize}
...
\end{itemize}
Similarly, we define the \defn{first critical value of a persistence module $V$} to be some $\beta_\degp \geq 0$ such that the structure maps before $\beta_\degp $ are isomorphism and $V_t$ changes isomorphism type at $t = \beta_\degp $.
...
For any $1 \leq p \leq n$, let $\theta$ be a cohomology operation of type $(\kk,n; \kk,\degp)$. 
