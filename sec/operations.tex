...
When $\pi$ is $\Z/p$ for some prime $p$, the algebra of stable cohomology operations, known as the mod $p$ \defn{Steenrod algebra}, is of central importance in homotopy theory.
...
\subsubsection{} The actual Steenrod algebra is generated by the \defn{Steenrod squares} $\set{\Sq^k}_{k \in \N}$, with $\Sq^k$ a degree $k$ stable operation and $\Sq^0 = \id$.
