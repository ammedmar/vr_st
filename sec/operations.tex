...
A cohomology operation $\theta \in \cO(\k,m; \k,n)$ is said to be \defn{linear} if $\k$ is a field and the map $\theta_{\cX} \colon \rH^m(\cX; \k) \to \rH^n(\cX; \k)$ is linear for any space $\cX$.
We denote the subset of $\cO(\k,m; \k,n)$ containing the linear cohomology operations by $\cO(m,n;\k)$.
...
\ling{I think $\img$ and $\ker$ should have the same format: either both with capitalized first letters or both with lower case letters}
\begin{align*}
...
\footnote{
We remind the reader that, for two pointed topological spaces, $\cX \vee \cY$ is the quotient of their disjoint union by the identification of basepoints.
Additionally, given two base point preserving maps $f \colon \cX_1 \to \cX_2$ and $g \colon \cY_1 \to \cY_2$ the map $(f \vee g) \colon \cX_1 \vee \cY_1 \to \cX_2 \vee \cY_2$ is defined by
\[
(f \vee g)(z) =
\begin{cases}
	f(z) & z \in \cX_1, \\
...
\subsubsection{}\ling{Should we say something about why we only consider Steenrod but not the general cohomology operations?}
