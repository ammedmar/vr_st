...
For every persistent space \(X\), the image and kernel of \(\theta\) define two persistent modules, denoted \(\img_\theta(X)\) and \(\ker_\theta(X)\), which we collectively refer to as \textit{persistent \(\theta\)-modules}.
When \(X\) is the Vietoris--Rips filtration \(\VR(\cX)\) of a metric space \(\cX\), we use the simplified notation \(\img_\theta^\VR(\cX)\) and \(\ker_\theta^\VR(\cX)\) for these.

We now present partial statements of two decomposition theorems for persistent \(\theta\)-modules, fully detailed and proven in \cref{ss:sums_products}.

\medskip\theorem For any two pointed metric spaces $\cX$ and $\cY$ and a linear cohomology operation \(\theta\):
\[
...
%\medskip In recent work, \cite{lim2024vietoris} established a deep connection between the Vietoris--Rips filtration of a metric space \(\cX\) and its Kuratowski embedding \(K \colon \cX \to \rL^\infty(\cX)\), which is the canonical isometric inclusion of \(\cX\) into a Banach space.
%%use by Gromov for the definition of the filling radius of closed connected Riemannian manifolds.
%More precisely, \cite{lim2024vietoris} proved that, if \(\rU_r(\cX)\) is the radius \(r\) neighborhood of \(K(\cX)\), then \(\VR_r(\cX)\) and \(\rU_r(\cX)\) are naturally homotopy equivalent for every \(r > 0\).
%
%Gromov's filling radius is the smallest \(r\) for which the fundamental class of a closed connected Riemanninan manifold is null-homologous in \(\rU_r(\cM)\).
%Using this invariant and other similar critical radii, construct examples of Riemannian pseudomanifolds for which the Gromov--Hausdorff estimate derived from the stability of persistent cohomology operations provide bounds that are tighter than those obtained through persistent homology.
%
%Specifically, let us consider the wedge of round spheres \(\VS = \bS^1 \vee \dots \vee \bS^n\), where each \(\bS^i\) has diameter \(\pi\).
%Additionally, we consider the real projective space \(\rp^n\), obtained as the quotient of a round sphere under the antipodal action, also with diameter \(\pi\).
...
Please consult \cref{...} for the precise inequalities.
